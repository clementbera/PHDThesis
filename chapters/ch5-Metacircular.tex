...
A frequent portion of code is going to be detected on the profiling counter, hence in the method \ct{Array >> do:}. The main issue is that an important part of the execution time is spent both in creating and evaluating the closure, and from the \ct{Array>>do:} method, it is not possible to know what closure will be executed unless its inlined into \ct{ApplicationClass >> exampleArrayLoop}. Hence, the optimiser is likely to decide to optimise \ct{ApplicationClass >> exampleArrayLoop} so it will be able to inline \ct{Array >> do:}, and then the closure evaluation (\ct{value:}), completely removing the need for the closure hence removing its allocation. \ct{Array >> do:} is a method, not a closure, and yet it is usually better to pick another function for optimisations.
